const stringFormat = (str, ...args) => {
    ...
    return str.replace(/{(\d+)}/g, (match, index) => typeof args[index] === 'undefined' ? match : args[index]);
};
...
class RouteManager {
    ...
    getLocation() {
        ...
        const location = {
            pathname: locationSplitArray[0] ? locationSplitArray[0].replace('#', '') : '/',
        ...
        pathSnippets.forEach((item, index) => {
            ...
            if(flag)
                for(const key in routes) {
                    ...
                    const pattern = new RegExp(`^${routes[key].url().replace(/:\w+/g, '([_\. @&a-zA-Z0-9-]+)')}$`);
                    const title = routes[key].title;
